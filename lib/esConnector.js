...
ESConnector.prototype.getESClientURL = function (settings) {
    ...
    settings.requestTimeout = (settings.requestTimeout || 25000);
...
ESConnector.prototype.connectToElasticClient = function (settings, dataSource) {
    this.elasticServer = this.es = new elasticsearch.Client(this.getESClientURL(settings));
...
function makeQuery(criteria, size, page) {
  ...
  opts['size'] = size || this.defaultSize;
  ...
  if(criteria) {
    ...
    var filter = { 'and': [] };
    filter.and = applyFilters.map(function(field) {
      var f = self.filters[field];
      f.terms[field] = criteria[field].split(',');
      return f;
    });
...
ESConnector.prototype.all = function all(model, filter, done) {
  var self = this;
  self.searchType = model;

  this.es.search(makeQuery.call(this, filter), function(err, res) {
    if (err) {
      console.trace(err.message);
    ...
    var result = {
      ...
      results: res.hits.hits.map(function (item) {
        return self.dataSourceToModel(model, item);
      }),
